// build/download-thumbs.js — fetch every record's thumbnail for CLIP embedding.
//
// Resumable (skips files already on disk) and gentle on the media host, which
// rate-limits bursts: a small concurrency pool + retry/backoff. Thumbs are tiny
// and CLIP downsizes to 224px anyway, so the thumbnail is plenty.
//
//   in:  build/records.json
//   out: build/thumbs/<id>.jpg            (gitignored)
//        build/thumbs-failed.json         ids that never came back
//
// Run:  node build/download-thumbs.js            (all)
//       LIMIT=200 node build/download-thumbs.js  (first 200 — for a rate test)
...
const url = (mid) => `https://media.tepapa.govt.nz/collection/${mid}/thumb`;
...
fs.writeFileSync(path.join(__dirname, 'thumbs-failed.json'), JSON.stringify(failed));
